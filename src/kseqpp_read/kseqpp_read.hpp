  ...
  public:
    ...
    inline bool read_sequence(Seq &rec, char &last_char) {
      ...
      while (c = this->next_char ? next_char : this->getc()) {
        ...
        this->getuntil(
          SEP::LINE, &rec.seq, &this->next_char, nullptr, rec.max_seq_size
        );
    ...
    inline void fetch_buffer() noexcept {
      this->buf_begin = 0;
      this->buf_end
        = this->load_buf(this->file_handle, this->buf, this->bufsize);
    }
    ...
    inline size_t get_idx_at_delim_or_bufend(
      SEP delimiter, string *str, const size_t max_str_size
    ) {
      size_t i;
      if (delimiter == SEP::LINE) {
        for (i = this->buf_begin;
             i < this->buf_end && this->buf[i] != '\n'
             && (str == nullptr || str->size() + (i - this->buf_begin) < max_str_size);
             ++i) {}
    ...
    inline void
    append_to_string(string *str, size_t *str_size, const size_t amount) {
      if (str != nullptr) { str->append(this->buf + this->buf_begin, amount); }
      if (str_size != nullptr) { *str_size += amount; }
    }

    inline void remove_last_element(string *str, size_t *str_size) {
